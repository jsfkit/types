 ...
 * the `<alternateUrls>` extension element inside `<externalBook>` (from
 * the 2021 extlinks2021 namespace).
 *
 * `absoluteUrl`/`relativeUrl` are captured from child elements of
 * `<alternateUrls>`; `driveId`/`itemId` are attributes on the
 * `<alternateUrls>` element itself.
 *
 ...
 * it in that case. Each subfield is independently optional per the extlinks2021
 * schema.
 *
 * @group Workbooks
 */
